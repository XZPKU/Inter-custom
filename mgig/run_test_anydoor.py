...
def main():
    ...
    trainer = pl.Trainer(gpus=n_gpus,strategy="ddp_sharded",precision=16, accelerator="gpu",callbacks=[logger,pl.callbacks.ModelCheckpoint(every_n_train_steps=40000, save_top_k=-1,save_weights_only=True)], progress_bar_refresh_rate=1, weights_save_path='./output/train',accumulate_grad_batches=accumulate_grad_batches)
