...
class HICOEvaluator():
    def __init__(self, preds, gts, dataset_path, out_dir, epoch, bins_num=10, use_nms=True, nms_thresh=0.5):
        ...
        self.hoi_list = json.load(open('/home/xuzhu/MoMA/data/hoi_list_new.json', 'r'))
        self.file_name_to_obj_cat = json.load(open(os.path.join('/home/xuzhu/MoMA/data', 'file_name_to_obj_cat.json'), "r"))
        self.nms_thresh = nms_thresh

        self.global_ids = self.annotations.keys()
        self.hoi_id_to_num = json.load(open(os.path.join('/home/xuzhu/MoMA/data', 'hoi_id_to_num.json'), "r"))
        self.rare_id_json = [key for key, item in self.hoi_id_to_num.items() if item['rare']]
        
        self.correct_mat = np.load(os.path.join('/home/xuzhu/MoMA/data', 'corre_hico.npy'))
    ...
    def load_gt_dets(self):
        ...
        anno_list = json.load(open(os.path.join('/home/xuzhu/MoMA/data', 'anno_list.json'), "r"))
    ...
    def evaluation_default(self):
        ...
        for ap, hoi_id in outputs:
            ...
            if not np.isnan(ap):
                count += 1
                map_ += ap
                category_specific_map[hoi_id] = ap
        ...
        return {"mAP_def": mAP['mAP'], "mAP_def_rare": mAP['mAP_rare'], "mAP_def_non_rare": mAP['mAP_non_rare'],"category_specific_map" : category_specific_map}
