...
def generate_caption(args):
    ...
    for i in range(1,601):
        all_mask_sotrage[i] = []
    for anno_item in anno_info:
        image_name = anno_item['file_name']
        # full_path = os.path.join(image_path,image_name)
        # image_content = Image.open(full_path)
        # image_array = np.array(image_content)
        instance_anno = anno_item['annotations']
        hoi_annotation = anno_item['hoi_annotation']
        
        img = Image.open(os.path.join(hico_image_path, anno_item['file_name'])).convert('RGB')
        w, h = img.size
        img_anno = anno_item
        ...
        classes = [(i, _valid_obj_ids.index(obj['category_id'])) for i, obj in
        ...
        for hoi_instance in hoi_annotation:
            ...
            pose_human_image_path = os.path.join(image_info_path,'anydoor_human_0.jpg')
            pose_human_image_mask_path = os.path.join(image_info_path,'anydoor_human_mask_0.jpg')
            # with open(info_path,'r') as f:
            #     cat_info = json.load(f)
            if not (os.path.exists(pose_human_image_path) and  os.path.exists(pose_human_image_mask_path)):
                print(f'skip {image_info_path} for no anydoor')
                continue
        
            if hoi_instance['subject_id'] not in kept_box_indices or hoi_instance['object_id'] not in kept_box_indices:
                continue
            ...
            o_fore = np.array(hoi_img.copy())
            o_fore[o_mask==0] = 255
                
            pose_human_image = Image.open(pose_human_image_path).convert('RGB')
            pose_h_mask = (cv2.imread(pose_human_image_mask_path) > 128).astype(np.uint8)[:,:,0]
            p_h_fore = np.array(pose_human_image.copy())
            p_h_fore[pose_h_mask==0] = 255
            resized_h = cv2.resize(p_h_fore, (128, 256))  # 注意顺序是 (宽, 高)
            resized_o = cv2.resize(o_fore, (128, 256))
            #combined_image = np.hstack((resized_h, resized_o))[:,:,::-1]
            combined_image = np.hstack((resized_h, resized_o))
            #print(combined_image.shape)  # 应该是 (256, 256, 3)
            output_path = './combined_image.png'  # 替换为实际保存路径
            cv2.imwrite(output_path, combined_image)
            # crop_pose_h_array = np.array(pose_human_image)[]
            ...
            hoi_splits = hoi_prompt[11:].split(' ')
            o_name = hoi_splits[-1:]
            query = f'There is a person and a {o_name[0]}, please use two sentences to seperately describe (1) the haircut, body shape and appearance of person,(2) the shape and appearance of object. Following is a example: The person is a short hair, tall and thin man, and wearing a hat. The bicycle is a colorful mountain bicycle.'
            # video_save_info = {}
            ...
            # images = load_images(image_files)
            images = [Image.fromarray(combined_image)]
            image_sizes = [x.size for x in images]
            images_tensor = process_images(
                images,
                image_processor,
            ...
            with open(os.path.join(image_info_path,'llava_caption_pose.txt'),'w') as f:    #设置文件对象
                f.write(outputs)  
    ...
    with open('/network_space/server128/shared/zhuoying/AnyDoor-main/datasets/mask_storage.json','w') as f:
