...
def generate_caption(args):
    ...
    hico_image_path = '/home/xuzhu/MoMA/data/process_data_test'
    ...
    all_num = 0  
    ans_list = []
    with open(os.path.join('/home/xuzhu/MoMA/data/hico_20160224_det/annotations/test_hico_ann.json')) as f:
        anno_info = json.load(f)
    all_mask_sotrage = {}
    # for i in range(1,601):
    #     all_mask_sotrage[i] = []
    ans_file_path = '/network_space/server128/shared/zhuoying/AnyDoor-main/datasets/all_llava_hico_test.json'
    ...
    for anno_item in tqdm(anno_info):
        ...
        origin_name = anno_item['file_name']
        image_folder = os.path.join(hico_image_path,f'{origin_name}_0')
        try:
            img = Image.open(os.path.join('/home/xuzhu/MoMA/data/hico_20160224_det/images/test2015', image_name)).convert('RGB')
            w, h = img.size
            img_anno = anno_item
            ...
            for hoi_instance in hoi_annotation:
                try:
                    hoi_instance_num += 1
                    if hoi_instance_num>2:
                        continue
                    image_info_path = os.path.join(root_path,f'{image_name}_{hoi_instance_num}')
                    
                    sample_path = image_info_path
                    sub_mask_path = os.path.join(sample_path,'human_mask.jpg')
                    obj_mask_path = os.path.join(sample_path,'object_mask.jpg')
                    sub_mask = (cv2.imread(sub_mask_path) > 128).astype(np.uint8)[:,:,0]
                    obj_mask = (cv2.imread(obj_mask_path) > 128).astype(np.uint8)[:,:,0]
                    union_mask = np.logical_or(sub_mask, obj_mask).astype(np.uint8)*255
                    union_mask_image = Image.fromarray(np.stack((union_mask,union_mask,union_mask),axis=2))
                    union_mask_image.save('./union_mask.jpg')
                    ...
                    query = f'Please judge whether the given mask  contains {hoi_prompt[11:]}, answer yes or no.'
                    # video_save_info = {}
                    ...
                    #     correct_num+=1
                    #     ans_list.append(f'{image_name}_{hoi_instance_num}')
                    #     ans
                    #all_num+=1
                    ans_file.write(json.dumps({"name":f'{image_name}_{hoi_instance_num}',"ans":outputs}))
                    ans_file.flush()
    ...
    # with open('/network_space/server128/shared/zhuoying/AnyDoor-main/LLaVA/llava/eval/mask_llava_judge_test_set.json','w') as f:
