...
def generate_caption(args):
    ...
    hico_image_path = '/home/xuzhu/MoMA/data/hico_20160224_det/images/test2015'
    ...
    for anno_item in anno_info:
        ...
        for hoi_instance in hoi_annotation:
            ...
            query = f'{hoi_prompt[11:]}'
            obj_name = query.split(' ')[-1]
            verb_name = query.split(' ')[-3]
            ans+=obj_name
            ans += ' '
            verb_ans+=verb_name
            verb_ans += ' '
        print(f'finish {image_name}') 
    with open('./hico_test_obj.txt','w') as f:
        f.write(ans)    
    with open('./hico_test_verb.txt','w') as f:
        f.write(verb_ans)    
