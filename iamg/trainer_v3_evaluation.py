...
class ImageCaptionSaver:
    ...
    def resize_forward(self,origin_image, target, condition, generation, h,w, seen,ids=None):
        ...
        save_path = os.path.join(self.base_path, str(seen).zfill(8) + '_target.png')
        torchvision.utils.save_image(target_origin_shape, save_path, nrow=self.nrow)
        save_path = os.path.join(self.base_path, str(seen).zfill(8) + '_condition.png')
    ...
    def resize_forward_eval_save(self,origin_image, target, condition, generation, h,w, seen,sample_name):
        ...
        self.base_path = './OUTPUT/eval'
        os.makedirs(os.path.join('./OUTPUT/eval',f'{sample_name}'),exist_ok=True)
        save_path = os.path.join(f'{self.base_path}/{sample_name}','mask.png')
        torchvision.utils.save_image(result_origin_shape, save_path, nrow=self.nrow)
...
class Trainer:
    def __init__(self, config, device="cuda"):
        ...
        #original_params_names.append(dino_state_dict.keys())
        ...
        #self.dino_encoder.eval()##already load satae dict at __init__, contains a projector trainable, so not set as eval mode
        self.autoencoder.eval()
        self.text_encoder.eval()
        # self.raw_path = '/home/xuzhu/MoMA/data/process_data/HICO_train2015_00001324.jpg_0'
        self.raw_path = '/home/xuzhu/MoMA/data/process_data/HICO_train2015_00000125.jpg_0'
    ...
    def generate_batch(self):
        ...
        raw_path = '/network_space/server128/shared/xuzhu/process_data/HICO_train2015_00005543.jpg_2'
        ...
        origin_h,origin_w = hoi_img.shape[:2]
        ...
        item['caption'] = 'a photo of a person playing skateboard'
        item['origin_h'] = origin_h
        item['origin_w'] = origin_w
        return item
    
    def generate_taobao_batch(self):
        raw_path = '/network_space/server128/shared/zhuoying/AnyDoor-main/taobao_video/20240406_1ed6f9b52607832f_456963067167_107031058147134_published_mp4_264_hd_taobao'
        hoi_img_path = os.path.join(raw_path,'img_1.png')
        sub_mask_path = os.path.join(raw_path,'1_human_mask.jpg')
        obj_mask_path = os.path.join(raw_path,'1_object_mask.jpg')
        ...
        item = self.hico_process_pairs(hoi_img,sub_mask,obj_mask,max_ratio = 1.0)
        item['caption'] = 'a photo of a person riding a bicycle'
        item['origin_h'] = origin_h
        item['origin_w'] = origin_w
        return item
    ...
    def save_ckpt_and_result(self):
        ...
        if not self.config.disable_inference_in_training:
            ...
            for i in range(batch_here):
                ...
                # target, condition  = self.dataset_train.vis_get_item(temp_data['target'],temp_data['condition'])
    ...
    def evaluation(self):
        ...
        if not self.config.disable_inference_in_training:
            ...
            #batch = self.generate_taobao_batch()
            batch_to_device(batch, self.device)
            real_target = []
            real_condition = []
            batch_here = 1
    ...
    @torch.no_grad()
    def evaluation_full(self):
        model_wo_wrapper = self.model.module if self.config.distributed else self.model
        total_sample_num = len(self.dataset_train)
        for i in tqdm(range(0,total_sample_num)):
            batch = self.dataset_train.__getitem__(i)
            #batch = self.generate_batch()
            #batch = self.generate_taobao_batch()
            batch_to_device(batch, self.device)
            
            ##### get real input image and taregt image##########
            real_target = []
            real_condition = []
            batch_here = 1
            for i in range(batch_here):
            ...
            iter_name=0
            origin_image  = Image.open(os.path.join(self.raw_path,'hoi.jpg'))
            self.image_caption_saver.resize_forward_eval_save(origin_image,real_target,real_condition,generation=samples, h= batch['origin_h'],w = batch['origin_w'],seen = iter_name,sample_name=batch['name'])
