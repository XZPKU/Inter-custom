...
class PLMSSampler_shape(object):
    def __init__(self, diffusion, model, schedule="linear", alpha_generator_func=None, set_alpha_scale=None):
        super().__init__()
        self.diffusion = diffusion
        self.model = model
    ...
    def plms_sampling(self, shape, input, uc=None, guidance_scale=1, mask=None, x0=None):
        ...
        if img == None:     
            img = torch.randn(shape, device=self.device, dtype=self.dtype)
            #input["x"] = torch.cat([img,condition],dim=1)
            input["x"] = img
    ...
    def p_sample_plms(self, input, t, index, guidance_scale=1., uc=None, old_eps=None, t_next=None):
        ...
        def get_model_output(input):
            e_t = self.model(input["x"],input["timesteps"],input["context"],input["condition"]) 
            if uc is not None and guidance_scale != 1:
                #unconditional_input = dict(x=input["x"], timesteps=input["timesteps"], context=uc, inpainting_extra_input=input["inpainting_extra_input"], grounding_extra_input=input['grounding_extra_input'])
                e_t_uncond = self.model( input["x"],input["timesteps"], uc,input["condition"]) 
                e_t = e_t_uncond + guidance_scale * (e_t - e_t_uncond)
            return e_t
...
class PLMSSampler_shape_bbox(object):
    def __init__(self, diffusion, model, schedule="linear", alpha_generator_func=None, set_alpha_scale=None):
        super().__init__()
        self.diffusion = diffusion
        self.model = model
    ...
    def p_sample_plms(self, input, t, index, guidance_scale=1., uc=None, old_eps=None, t_next=None):
        ...
        def get_model_output(input):
            e_t = self.model(input["x"],input["timesteps"],input["context"]) 
            if uc is not None and guidance_scale != 1:
                #unconditional_input = dict(x=input["x"], timesteps=input["timesteps"], context=uc, inpainting_extra_input=input["inpainting_extra_input"], grounding_extra_input=input['grounding_extra_input'])
                e_t_uncond = self.model( input["x"],input["timesteps"], uc) 
                e_t = e_t_uncond + guidance_scale * (e_t - e_t_uncond)
            return e_t
