...
class BasicTransformerBlock_fore(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=True):
        super().__init__()
        self.attn1 = SelfAttention(query_dim=query_dim, heads=n_heads, dim_head=d_head)
        self.ff = FeedForward(query_dim, glu=True)
        ...
        if fuser_type == "gatedSA":
            # note key_dim here actually is context_dim
            self.fuser = GatedSelfAttentionDense(query_dim, key_dim, n_heads, d_head)
            #self.image_fuser = GatedSelfAttentionDense(query_dim, key_dim, n_heads)
        elif fuser_type == "gatedSA2":
            # note key_dim here actually is context_dim
            self.fuser = GatedSelfAttentionDense2(query_dim, key_dim, n_heads, d_head)
            #self.image_fuser = GatedSelfAttentionDense2(query_dim, key_dim, n_heads, d_head)
        elif fuser_type == "gatedCA":
            self.fuser = GatedCrossAttentionDense(query_dim, key_dim, value_dim, n_heads, d_head)
            #self.image_fuser = GatedCrossAttentionDense(query_dim, key_dim, value_dim, n_heads, d_head)
        else:
            assert False
        # if fuser_type == "gatedSA":
        #     # note key_dim here actually is context_dim
        ...
        #     assert False
        self.image_fuser = GatedCrossAttentionDense(query_dim, key_dim, value_dim, n_heads, d_head)
    def forward(self, x, context, objs, objs_fore):
        #        return checkpoint(self._forward, (x, context, objs), self.parameters(), self.use_checkpoint)
        if self.use_checkpoint and x.requires_grad:
            return checkpoint.checkpoint(self._forward, x, context, objs, objs_fore)
        else:
            return self._forward(x, context, objs, objs_fore)

    def _forward(self, x, context, objs, objs_fore):
        x = self.attn1(self.norm1(x)) + x
        x = self.fuser(x, objs)  # identity mapping in the beginning
        x = self.image_fuser(x,objs_fore)
        x = self.attn2(self.norm2(x), context, context) + x  
        x = self.ff(self.norm3(x)) + x
        return x

# class BasicTransformerBlock_fore_only_dino(nn.Module):
#     def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=True):
#         super().__init__()
#         self.attn1 = SelfAttention(query_dim=query_dim, heads=n_heads, dim_head=d_head)
#         self.ff = FeedForward(query_dim, glu=True)
...
class HOISpatialTransformer_fore(nn.Module):
    def __init__(self, in_channels, key_dim, value_dim, n_heads, d_head, depth=1, fuser_type=None, use_checkpoint=True):
        super().__init__()
        self.in_channels = in_channels
        query_dim = n_heads * d_head
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock_fore(query_dim, key_dim, value_dim, n_heads, d_head, fuser_type,
                                   use_checkpoint=use_checkpoint)
             for d in range(depth)]
        )
    ...
    def forward(self, x, context, objs, objs_fore):
        ...
        x = rearrange(x, 'b c h w -> b (h w) c')  # .contiguous()
        for block in self.transformer_blocks:
            x = block(x, context, objs, objs_fore)
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
        x = self.proj_out(x)
        return x + x_in
