...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="/network_space/server128/shared/zhuoying/models/clip-vit-large-patch14", device="cuda", max_length=77):
        super().__init__()
...
config_path = './configs/hoi_hico_text_add_dino.yaml'
config = OmegaConf.load(config_path)
#DINOv2_weight_path = config.model.params.cond_stage_config.weight
DINOv2_weight_path = config.model.params.dino_encoder.weight
class FrozenDinoV2Encoder(AbstractEncoder):
    ...
    def forward(self, image):
        ...
        hint = torch.cat([image_features,tokens],1) # 8,257,768
        hint = self.projector(hint)
        #assert hint.shape == torch.Size([4,257,768])
        return hint
