...
from ldm.modules.attention import HOISpatialTransformer, HOISpatialTransformer_fore
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, objs,objs_fore):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, HOISpatialTransformer):
                x = layer(x, context, objs)
            elif isinstance(layer, HOISpatialTransformer_fore):
                x = layer(x,context,objs,objs_fore)
            else:
                x = layer(x)
        return x
...
class HOIUNetModel_add_dino(nn.Module):
    def __init__(
            self,
            image_size,
            in_channels,
            ...
            grounding_tokenizer=None,
            dino_encoder=None,
            image_grounding_tokenizer=None,
    ):
        super().__init__()
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    dim_head = ch // num_heads
                    layers.append(HOISpatialTransformer_fore(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads,
                                                        d_head=dim_head, depth=transformer_depth, fuser_type=fuser_type,
                                                        use_checkpoint=use_checkpoint))
        ...
        self.middle_block = TimestepEmbedSequential(
            ResBlock(ch,
                     ...
                     use_checkpoint=use_checkpoint,
                     use_scale_shift_norm=use_scale_shift_norm),
            HOISpatialTransformer_fore(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads, d_head=dim_head,
                               depth=transformer_depth, fuser_type=fuser_type, use_checkpoint=use_checkpoint),
            ResBlock(ch,
                     time_embed_dim,
                     dropout,
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                if ds in attention_resolutions:
                    dim_head = ch // num_heads
                    layers.append(HOISpatialTransformer_fore(ch, key_dim=context_dim, value_dim=context_dim,
                                                     n_heads=num_heads, d_head=dim_head, depth=transformer_depth,
                                                     fuser_type=fuser_type, use_checkpoint=use_checkpoint))
                if level and i == num_res_blocks:
                    out_ch = ch
        ...
        self.position_net = instantiate_from_config(grounding_tokenizer)
        self.dino_encoder = instantiate_from_config(dino_encoder)
        self.image_position_net = instantiate_from_config(image_grounding_tokenizer)
    def restore_first_conv_from_SD(self):
        if self.first_conv_restorable:
            device = self.input_blocks[0][0].weight.device
            dtype = self.input_blocks[0][0].weight.dtype
    ...
    def forward(self, input):
        ...
        # Grounding tokens: B*N*C
        # objs = self.position_net(**grounding_input)
        objs = self.position_net(grounding_input['subject_boxes'],grounding_input['object_boxes'],grounding_input['masks'],grounding_input['subject_positive_embeddings'],grounding_input['object_positive_embeddings'],grounding_input['action_positive_embeddings'])
        sub_fore_features = self.dino_encoder(grounding_input['subject_fore_embeddings'].reshape(-1,3,224,224)).reshape(objs.shape[0],1,257,768)
        obj_fore_features = self.dino_encoder(grounding_input['object_fore_embeddings'].reshape(-1,3,224,224)).reshape(objs.shape[0],1,257,768)
        objs_fore = self.image_position_net(grounding_input['subject_boxes'],grounding_input['object_boxes'],grounding_input['masks'],sub_fore_features,obj_fore_features)
        # Time embedding
        t_emb = timestep_embedding(input["timesteps"], self.model_channels, repeat_only=False)
        t_emb = t_emb.to(dtype=objs.dtype)  # change to 16bit if model is at 16
        emb = self.time_embed(t_emb)
        ...
        hs = []
        for module in self.input_blocks:
            h = module(h, emb, context, objs, objs_fore)
            hs.append(h)

        h = self.middle_block(h, emb, context, objs, objs_fore)
