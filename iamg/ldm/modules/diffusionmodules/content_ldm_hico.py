...
class CondModel(UNetModel):
    ...
    #     )
    #     return out
    def forward(self, x, timesteps, context,condition):
        # if inpaint_image is None:
        #     inpaint_image = torch.zeros_like(x)
        # if inpaint_mask is None:
        #     inpaint_mask = torch.zeros_like(x[:, :1])
...
#         # target, condition,caption = batch
#         target = batch['target']
#         condition = batch['condition']
#         #caption = self.get_learned_conditioning(batch['caption']).to(self.device)
#         caption = batch['caption']
#         target = self.encode(target.float().to(self.device))    
#         condition = self.encode(condition.float().to(self.device))
